...
step=.1;W=1410;H=1634;N=W*H;obs=[];drawables=[]
...
def candidates(pt,ls,blocked,net):
 ...
 for li in ls:
  for dx,dy in {(round(rad*math.cos(ang)/step),round(rad*math.sin(ang)/step)) for rad in [0,.2,.4,.6,.8,1,1.4,1.8,2.2] for ang in [i*math.pi/8 for i in range(16)]}:
    x,y=cx+dx,cy+dy
    if not (0<=x<W and 0<=y<H) or blocked[li,y,x]:continue
    q=(x*step,y*step);sh=k.SHAPE_SEGMENT(vec(*pt),vec(*q),k.FromMM(.25))
...
for count,block in enumerate(blocks):
 ...
 net=b.FindNet(name).GetNetCode();a=tuple(map(float,ends[0][:2]));z=tuple(map(float,ends[1][:2]));blocked=raster(net,.376);vfree=~raster(net,.65).any(axis=0)
 ...
 for u,v in zip(compact,compact[1:]):
  ...
  if u[0]!=v[0]:
   ...
   sh=k.SHAPE_SEGMENT(vec(*u[1:]),vec(*v[1:]),k.FromMM(.25));okay &= clear(sh,net,u[0]);pieces.append(('track',u,v))
 ...
 for kind,u,v in pieces:
  if kind=='via':
   ...
   t=k.PCB_TRACK(b);t.SetStart(vec(*u[1:]));t.SetEnd(vec(*v[1:]));t.SetWidth(k.FromMM(.25));t.SetLayer(cu[u[0]])
  t.SetNetCode(net);b.Add(t);additem(t)
 added.append(name);print('CONNECTED',name,'total',len(added),flush=True)
 if len(added)%10==0:k.SaveBoard(str(f),b)
k.ZONE_FILLER(b).Fill(b.Zones());k.SaveBoard(str(f),b)
(p/'reports/Fine_routes.json').write_text(json.dumps({'added':added,'failed':fails},indent=2));print('FINISHED',len(added),len(fails),flush=True)
