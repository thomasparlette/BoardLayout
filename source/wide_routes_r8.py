...
for block in re.split(r'(?=^\[)',(p/'reports/After_routing_DRC.txt').read_text(),flags=re.M):
 ...
 blocked=raster(net,.251+width/2);basefree=~raster(net,.651).any(axis=0);offsets=[(-.6,-.6),(-.6,.6),(.6,-.6),(.6,.6)] if width>=1.2 else [(0,0)];vfree=basefree.copy()
 for dx,dy in offsets:vfree &= np.roll(np.roll(basefree,round(dx/step),axis=1),round(dy/step),axis=0)
...
k.ZONE_FILLER(b).Fill(b.Zones());k.SaveBoard(str(f),b);(p/'reports/R8_power_routes.json').write_text(json.dumps({'added':added,'failed':failed,'ampacity':'UNQUALIFIED'},indent=2));print('FINISHED',len(added),len(failed),flush=True)
