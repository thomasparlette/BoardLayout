...
(p/'reports/Geometry_current_review.json').write_text(json.dumps({'status':'NOT an ampacity calculation or thermal qualification','copper_assumption_um_by_layer':{'F.Cu':35,'In1.Cu-In6.Cu':17.5,'B.Cu':35},'pth_ring_screen_threshold_mm':.254,'pth_ring_candidates_below_threshold':rings,'track_lengths_by_net_layer_width_mm':dict(lengths),'segments_below_nominal_trunk_target':dict(necks),'notes':['Parallel traces must be reviewed as complete networks; simply adding their widths does not establish current sharing.','Injector basis: 0.7 A maximum RMS and 1.2 A design peak, one active channel. Ignition basis: 1.5 A peak treated conservatively as continuous, one active channel.','Signal traces use the separately stated 0.20 mm default; 0.25 mm inventory target is not a current rating.','The hole screen is conservative; verify actual package lead dimensions and manufacturer stackup-specific rules.']},indent=2))
