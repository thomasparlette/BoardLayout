...
for dx,dy in sorted([(x,y) for x in range(-15,16) for y in range(-5,21)],key=lambda a:a[0]*a[0]+a[1]*a[1]):
 pos=(78+dx,132+dy);f.SetPosition(vec(*pos));remove={};valid=True
...
report={'ref':'D33','from':{'side':'B.Cu','xy':old},'to':{'side':'B.Cu','xy':pos},'displaced_signal_tracks':len(remove),'reason':'Move the EPC clamp diode near the transmission pass-through to provide short accessible branches. Package and thermal rating remain unqualified.'};(p/'reports/D33_placement_R10.json').write_text(json.dumps(report,indent=2));print(report)
