...
bare=trimesh.creation.extrude_polygon(plate,T,engine='earcut');bare.export(out/'R10_Bare_Board_1p6mm.stl')
pieces=[bare];render=[]
for part in g['parts']:
...
assembly=trimesh.boolean.union(pieces,engine='manifold');assembly.export(out/'R10_Component_Envelopes.stl')
...
d.text((30,20),'R10 — MECHANICAL FIT TRIAL',fill='#122536',font=f)
...
notes=['Board: 140.93 x 163.36 x 1.60 mm','Eight-layer routing candidate','D26-D28, D30, D33: moved underneath','Front / factory connector: y = 0','Rear / Micro-Fit mating face: y = 163.36','','J120: Molex 43045-2000','Locating holes: 3.00 mm nominal','20 electrical contacts; 3.00 mm pitch','','J130 moved to (62,146)','J140 moved to (90,140)','J131 retained at (62,156)','','Orange: assumed / approximate body','Green: top generic package; blue: underside','','NOT MODELLED:','MS3 daughtercard above its socket','MapDaddy module and hose fittings','Ford connector shell and bent pins','Micro-Fit cable plug and latch travel','Case thermal clips / insulation','','Seven screw holes are 3.20 mm trial sizes.','Print at 100% in millimetres.','Check the bare board before the tall model.','','ROUTING / ELECTRICAL STATUS:','See current README and native reports.','Not released for manufacturing.']
for i,t in enumerate(notes):d.text((850,140+i*24),t,font=sm if len(t)>43 else ImageFont.truetype(font,17),fill='#243849')
im.save(out/'R10_Mechanical_Layout.png')
...
d.text((35,30),'R10 COMPONENT ENVELOPES — TOP VIEW',font=f,fill='#162837')
...
im.save(out/'R10_Envelope_Preview.png');print(json.dumps(check))
...
for _,tri,col in sorted(faces,key=lambda a:a[0]):d.polygon(tri,fill=col)
d.text((35,30),'R10 COMPONENT ENVELOPES — UNDERSIDE VIEW',font=f,fill='#162837')
...
im.save(out/'R10_Underside_Preview.png')
