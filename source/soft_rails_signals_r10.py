...
protectedcodes=powercodes-{b.FindNet(n).GetNetCode() for n in ['DGND','VCC_5']}
...
for block in blocks:
 ...
 if name not in {n for n,reason in json.loads((p/'reports/R10_negotiated_dgnd_signal.json').read_text())['failed']}:continue
...
k.ZONE_FILLER(b).Fill(b.Zones());k.SaveBoard(str(f),b);(p/'reports/R10_negotiated_rails_signal.json').write_text(json.dumps({'added':added,'failed':failed,'displaced_items':ripped},indent=2));print('DONE',len(added),'displaced',len(ripped),flush=True)
