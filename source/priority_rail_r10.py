...
prior=p/'reports/R10_priority_rail.json';prior_data=json.loads(prior.read_text()) if prior.exists() else {};protected=set(prior_data.get('protected',prior_data.get('added',[]))) | (powernames-{'VCC_5','DGND'})
nt=[]
...
k.ZONE_FILLER(b).Fill(b.Zones());k.SaveBoard(str(f),b);(p/'reports/R10_priority_rail.json').write_text(json.dumps({'added':added,'failed':failed,'displaced_items':ripped,'protected':sorted(protected)},indent=2));print('DONE',len(added),'displaced',len(ripped),flush=True)
