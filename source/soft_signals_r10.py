...
for block in blocks:
 ...
 if any(t.GetNetCode() in powercodes or t.IsLocked() for t in targets.values()):failed.append((name,'protected power collision'));continue
...
k.ZONE_FILLER(b).Fill(b.Zones());k.SaveBoard(str(f),b);(p/'reports/R10_negotiated_signal.json').write_text(json.dumps({'added':added,'failed':failed,'displaced_items':ripped},indent=2));print('DONE',len(added),'displaced',len(ripped),flush=True)
