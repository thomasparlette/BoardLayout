...
bare=trimesh.creation.extrude_polygon(plate,T,engine='earcut');bare.export(out/'R7_Bare_Board_1p6mm.stl')
pieces=[bare];render=[]
for part in g['parts']:
...
assembly=trimesh.boolean.union(pieces,engine='manifold');assembly.export(out/'R7_Component_Envelopes.stl')
...
d.text((30,20),'R7 — MECHANICAL FIT TRIAL',fill='#122536',font=f)
...
im.save(out/'R7_Mechanical_Layout.png')
...
d.text((35,30),'R7 COMPONENT ENVELOPES — TOP VIEW',font=f,fill='#162837')
...
im.save(out/'R7_Envelope_Preview.png');print(json.dumps(check))
...
for _,tri,col in sorted(faces,key=lambda a:a[0]):d.polygon(tri,fill=col)
d.text((35,30),'R7 COMPONENT ENVELOPES — UNDERSIDE VIEW',font=f,fill='#162837')
d.text((35,68),'43 underside component bodies; lead protrusions and case contact geometry are not included.',font=sm,fill='#953d19')
im.save(out/'R7_Underside_Preview.png')
