...
for count,block in enumerate(blocks):
 ...
 net=b.FindNet(name).GetNetCode();a=tuple(map(float,ends[0][:2]));z=tuple(map(float,ends[1][:2]));blocked=raster(net,.301);vfree=~raster(net,.501).any(axis=0)
 ...
 for u,v in zip(compact,compact[1:]):
  if u==v:continue
  if u[0]!=v[0]:
   sh=k.SHAPE_CIRCLE(vec(*u[1:]),k.FromMM(.3));okay &= clear(sh,net,None);pieces.append(('via',u,v))
  else:
 ...
 for kind,u,v in pieces:
  if kind=='via':
   near=next((q for q in newvia if math.hypot(u[1]-q[1],u[2]-q[2])<.551),None)
   if near is not None:
    for li in {u[0],v[0]}:
     aa=(li,*u[1:]);zz=(li,*near[1:])
     if aa!=zz:
      sh=k.SHAPE_SEGMENT(vec(*aa[1:]),vec(*zz[1:]),k.FromMM(.2));okay &= clear(sh,net,li);cleaned.append(('track',aa,zz))
    continue
   newvia.append(u)
