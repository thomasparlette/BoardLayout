"""Power/rail routing experiment. Wide trunks, recorded short pad escapes.
Four-via arrays for load-net transitions. Current capacity remains unqualified.
"""
from pathlib import Path
import pcbnew as k, numpy as np
from PIL import Image,ImageDraw
import re,json,heapq,math,time
...
step=.2;W=705;H=817;N=W*H;obs=[];drawables=[]
...
def additem(item):
 for li,l in enumerate(cu):
  ...
  else:drawables.append((item.GetNetCode(),li,'rect',(mm(bb.GetX()),mm(bb.GetY()),mm(bb.GetRight()),mm(bb.GetBottom())),0))
...
def clear(shape,net,li):
 ...
 for n,l,o,box in obs:
  if n!=net and (li is None or l==li) and bb.Intersects(box) and k.SHAPE.Collide(shape,o,k.FromMM(.251)):return False
 for z in b.Zones():
  if z.GetIsRuleArea() and k.SHAPE.Collide(shape,z.Outline(),k.FromMM(.251)):return False
 return True
def raster(net,r):
 ims=[Image.new('1',(W,H),0) for _ in cu];ds=[ImageDraw.Draw(i) for i in ims]
 for n,l,kind,coords,width in drawables:
  if n==net:continue
  d=ds[l];x,y,xx,yy=coords
  if kind=='rect':d.rectangle((math.floor((x-r)/step),math.floor((y-r)/step),math.ceil((xx+r)/step),math.ceil((yy+r)/step)),fill=1)
  else:
   rad=width/2+r;d.line((round(x/step),round(y/step),round(xx/step),round(yy/step)),fill=1,width=math.ceil(2*rad/step)+1)
...
def end_layers(desc):
 if 'PTH pad' in desc or 'Via [' in desc:return list(range(4))
...
def candidates(pt,ls,blocked,net):
 ...
 for li in ls:
  for dx,dy in {(round(rad*math.cos(ang)/step),round(rad*math.sin(ang)/step)) for rad in [0,.4,.8,1.2,1.6,2,2.4,3] for ang in [i*math.pi/8 for i in range(16)]}:
    x,y=cx+dx,cy+dy
    if not (0<=x<W and 0<=y<H) or blocked[li,y,x]:continue
    q=(x*step,y*step);sh=k.SHAPE_SEGMENT(vec(*pt),vec(*q),k.FromMM(neck))
...
for count,block in enumerate(blocks):
 ...
 neck=min(width,.8)
 offsets=[(-.6,-.6),(-.6,.6),(.6,-.6),(.6,.6)] if width>=1.2 else [(0,0)]
 net=b.FindNet(name).GetNetCode();a=tuple(map(float,ends[0][:2]));z=tuple(map(float,ends[1][:2]));blocked=raster(net,.25+width/2);vfree=~raster(net,1.25 if width>=1.2 else .65).any(axis=0)
 ...
 starts=candidates(a,end_layers(ends[0][2]),blocked,net);goals=candidates(z,end_layers(ends[1][2]),blocked,net)
 ...
 for i in range(1,len(points)-1):
  ...
  if u[0]==v[0]==w[0] and ((u[1]==v[1]==w[1]) or (u[2]==v[2]==w[2])):continue
  compact.append(v)
 compact.append(points[-1]);compact=[(compact[0][0],*a)]+compact+[(compact[-1][0],*z)]
 pieces=[];okay=True
 for pi,(u,v) in enumerate(zip(compact,compact[1:])):
  if u==v:continue
  if u[0]!=v[0]:
   for dx,dy in offsets:
    q=(u[0],u[1]+dx,u[2]+dy);sh=k.SHAPE_CIRCLE(vec(*q[1:]),k.FromMM(.4));okay &= clear(sh,net,None);pieces.append(('via',q,q,0))
    for layer in [u[0],v[0]]:
     start=(layer,*u[1:]);end=(layer,*q[1:]);sh=k.SHAPE_SEGMENT(vec(*start[1:]),vec(*end[1:]),k.FromMM(neck));okay &= clear(sh,net,layer)
     if start!=end:pieces.append(('track',start,end,neck))
  else:
   w=neck if pi in [0,len(compact)-2] else width
   sh=k.SHAPE_SEGMENT(vec(*u[1:]),vec(*v[1:]),k.FromMM(w));okay &= clear(sh,net,u[0]);pieces.append(('track',u,v,w))
 ...
 added.append({'net':name,'trunk_width':width,'escape_width':neck,'pieces':pieces});print('CONNECTED',name,'total',len(added),flush=True)
 if len(added)%10==0:k.SaveBoard(str(f),b)
k.ZONE_FILLER(b).Fill(b.Zones());k.SaveBoard(str(f),b)
(p/'reports/Power_routes.json').write_text(json.dumps({'status':'UNQUALIFIED CURRENT CAPACITY; inspect pad escapes and array transitions','added':added,'failed':fails},indent=2));print('FINISHED',len(added),len(fails),flush=True)
