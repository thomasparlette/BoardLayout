...
for block in blocks:
 ...
 if name not in ['COIL_1','COIL_2','COIL_3','COIL_4','VPWR']:continue
 ...
 if len(added)>=7:break
k.ZONE_FILLER(b).Fill(b.Zones());k.SaveBoard(str(f),b);(p/'reports/R9_critical_power.json').write_text(json.dumps({'added':added,'failed':failed,'displaced_items':ripped},indent=2));print('DONE',len(added),'displaced',len(ripped),flush=True)
