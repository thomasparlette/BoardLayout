...
for block in re.split(r'(?=^\[)',(p/'reports/After_routing_DRC.txt').read_text(),flags=re.M):
 ...
 print('TRY RAIL',name,flush=True)
 neck=min(width,.6);net=b.FindNet(name).GetNetCode();a=tuple(map(float,ends[0][:2]));z=tuple(map(float,ends[1][:2]));b.BuildConnectivity();ia=island(a,net,end_layers(ends[0][2]));iz=island(z,net,end_layers(ends[1][2]))
...
k.ZONE_FILLER(b).Fill(b.Zones());k.SaveBoard(str(f),b);(p/'reports/R9_wide_routes.json').write_text(json.dumps({'added':added,'failed':failed,'ampacity':'UNQUALIFIED'},indent=2));print('FINISHED',len(added),len(failed),flush=True)
