...
for fp in b.GetFootprints():
 if fp.GetReference().startswith('NT'):
  ...
  for a,c in [(x-2,y-1),(x+2,y-1),(x+2,y+1),(x-2,y+1)]:sh.Append(k.FromMM(a),k.FromMM(c))
  bb=sh.BBox();obs.append((-1,0,sh,bb))
  ...
  drawables.append((-1,0,'rect',(x-2,y-1,x+2,y+1),0))
...
def seeds(items,blocked,net,neck):
 ...
 return out
added=[];failed=[]
name='PGND';net=b.FindNet(name).GetNetCode()
...
for distance,i,j in pairs:
 ...
 width=4.0 if power(ga) and power(gz) else .25;neck=min(width,.6)
 print('GROUND BRIDGE',i,j,'width',width,'separation',distance,flush=True)
 blocked=raster(net,.251+width/2);vfree=~raster(net,1.51 if width>=1.2 else .551).any(axis=0);offsets=[(-.6,-.6),(-.6,.6),(.6,-.6),(.6,.6)] if width>=1.2 else [(0,0)]
 ...
 for item in [pd for fp in b.GetFootprints() for pd in fp.Pads()]+[t for t in b.GetTracks() if isinstance(t,k.PCB_VIA)]:
  ...
  if drill<=0:continue
  x,y=mm(item.GetPosition().x),mm(item.GetPosition().y);r=drill/2+.15+.251
 ...
 for i,(u,v) in enumerate(zip(compact,compact[1:])):
  ...
  if u[0]!=v[0]:
   for dx,dy in offsets:
    q=(u[0],u[1]+dx,u[2]+dy);okay &= clear(k.SHAPE_CIRCLE(vec(*q[1:]),k.FromMM(.3)),net,None);pieces.append(('via',q,q,0))
    for l in [u[0],v[0]]:
 ...
 for kind,u,v,w in pieces:
  ...
  if any(math.hypot(u[1]-q[1],u[2]-q[2])<.551 for q in newvias):okay=False;break
  newvias.append(u)
 if not okay:failed.append((name,'via spacing'));continue
 for kind,u,v,w in pieces:
  if kind=='via':
...
k.ZONE_FILLER(b).Fill(b.Zones());k.SaveBoard(str(f),b);(p/'reports/Ground_small_bridges_R9.json').write_text(json.dumps({'added':added,'failed':failed,'groups_remaining':len({root(i) for i in range(len(groups))}),'ampacity':'UNQUALIFIED'},indent=2));print('DONE GROUND',len(added),'groups',len({root(i) for i in range(len(groups))}),flush=True)
