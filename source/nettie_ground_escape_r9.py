...
for ref,pin,name in [('NT1','1','PGND')]:
 ...
 for dx in range(-30,31,2):
  for dy in range(-30,31,2):
   ...
   if x>origin[0]-1.5:continue
   v=vec(x,y)
   if not any(z.HitTestFilledArea(target,v) for z in zones):continue
   via=k.SHAPE_CIRCLE(v,k.FromMM(.3));track=k.SHAPE_SEGMENT(pos,vec(x,origin[1]),k.FromMM(.25));track2=k.SHAPE_SEGMENT(vec(x,origin[1]),v,k.FromMM(.25));shapes=[(via,None),(track,layer),(track2,layer)];bad=False;cuts={}
 ...
 t=k.PCB_TRACK(b);t.SetStart(pos);t.SetEnd(vec(x,origin[1]));t.SetWidth(k.FromMM(.25));t.SetLayer(layer);t.SetNetCode(net);b.Add(t)
 t=k.PCB_TRACK(b);t.SetStart(vec(x,origin[1]));t.SetEnd(vec(x,y));t.SetWidth(k.FromMM(.25));t.SetLayer(layer);t.SetNetCode(net);b.Add(t)
...
k.ZONE_FILLER(b).Fill(b.Zones());k.SaveBoard(str(f),b);(p/'reports/Nettie_ground_escape_R9.json').write_text(json.dumps({'added':done,'failed':failed},indent=2))
