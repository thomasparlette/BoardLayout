...
prior=p/'reports/R10_repair_injectors.json';prior_data=json.loads(prior.read_text()) if prior.exists() else {};protected=set(prior_data.get('protected',prior_data.get('added',[]))) | (powernames-{'VCC_5','DGND'})
nt=[]
...
def power_seeds(items,blocked,net):
 seeds={};queue=deque();prev={};orig={};dist={};components=set()
 ...
 while queue:
  ...
  if reachable[node] and int(reachable[node]) not in components and not blocked[li,y,x]:
   path=[];n=node
   ...
   components.add(int(reachable[node]))
   if wanted_components is not None and int(reachable[node]) in wanted_components:return seeds
  if reachable[node] and component_sizes[int(reachable[node])]>500000:continue
...
for block in blocks:
 ...
 if name not in ['INJECTOR_7','INJECTOR_8']:continue
 ...
 if name=='VPWR':
  fuse=next(pd for fp in b.GetFootprints() if fp.GetReference()=='F1' for pd in fp.Pads() if pd.GetNumber()=='1');fid=fuse.m_Uuid.AsString()
  padsets=[{it.m_Uuid.AsString() for it in group.values() if isinstance(it,k.PAD)} for group in [ia,iz]]
  if any(ids=={fid} for ids in padsets):
   width=1.0;neck=.6;offsets=[(0,0)];print('F1-only branch: 1.0 mm target to specified 1 A fuse; main VPWR target remains 4.0 mm',flush=True)
 if name=='ACTUATOR_12':
  diode=next(pd for fp in b.GetFootprints() if fp.GetReference()=='D33' for pd in fp.Pads() if pd.GetNumber()=='1');did=diode.m_Uuid.AsString()
  padsets=[{it.m_Uuid.AsString() for it in group.values() if isinstance(it,k.PAD)} for group in [ia,iz]]
  if any(ids=={did} for ids in padsets):
   width=widths['TRANS_EPC'];neck=.6;offsets=[(-.6,-.6),(-.6,.6),(.6,-.6),(.6,.6)];print('D33-only return branch: matches EPC width; shared ACTUATOR trunk remains 4.0 mm',flush=True)
 ...
 data=struct.pack('<II',W,H)+blocked.astype('uint8').tobytes()+vfree.astype('uint8').tobytes();reachable=np.frombuffer(subprocess.run([str(p/'source/regions_r10')],input=data,stdout=subprocess.PIPE,check=True).stdout,dtype=np.uint32)
 escape_paths={};component_sizes=np.bincount(reachable);wanted_components=None;starts=power_seeds(ia,blocked,net);wanted_components={int(reachable[n]) for n in starts};goals=power_seeds(iz,blocked,net)
 common={int(reachable[n]) for n in starts}&{int(reachable[n]) for n in goals}
 print('REGIONS',name,'start',len(starts),'goal',len(goals),'shared',len(common),a,z,flush=True)
 starts={n:v for n,v in starts.items() if int(reachable[n]) in common};goals={n:v for n,v in goals.items() if int(reachable[n]) in common}
 if not starts or not goals:failed.append((name,'hard escape / no common wide component'));continue
...
k.ZONE_FILLER(b).Fill(b.Zones());k.SaveBoard(str(f),b);(p/'reports/R10_repair_injectors.json').write_text(json.dumps({'added':added,'failed':failed,'displaced_items':ripped,'protected':sorted(protected)},indent=2));print('DONE',len(added),'displaced',len(ripped),flush=True)
