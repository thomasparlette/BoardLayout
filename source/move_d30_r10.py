...
p=Path(__file__).resolve().parents[1];fn=p/'GrandMarquis97_RevA.kicad_pcb';b=k.LoadBoard(str(fn));mm=k.ToMM;vec=lambda x,y:k.VECTOR2I(k.FromMM(x),k.FromMM(y));f=next(x for x in b.GetFootprints() if x.GetReference()=='D30');old=(mm(f.GetPosition().x),mm(f.GetPosition().y));power={n for d in json.load(open(p/'reports/Routing_targets.json'))['widths'].values() if d['width_mm']>.25 for n in d['nets']};cell=k.FromMM(3);bins=collections.defaultdict(list);holes=[]
...
for dx,dy in sorted([(x,y) for x in range(-4,5) for y in range(-3,4)],key=lambda a:a[0]*a[0]+a[1]*a[1]):
 pos=(90+dx,130+dy);f.SetPosition(vec(*pos));remove={};valid=True
...
report={'ref':'D30','from':{'side':'F.Cu','xy':old},'to':{'side':'B.Cu','xy':pos},'displaced_signal_tracks':len(remove),'reason':'Relocate the brake-input diode out of a trapped front-side routing area. Package and thermal rating remain unqualified.'};(p/'reports/D30_placement_R10.json').write_text(json.dumps(report,indent=2));print(report)
