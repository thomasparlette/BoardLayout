...
int main(){ios::sync_with_stdio(false);cin.tie(nullptr);
 ...
 while(!q.empty()){
  ...
  if((++visits%16384)==0 && chrono::duration<double>(chrono::steady_clock::now()-start).count()>15)break;
  uint32_t local=n%N,x=local%W,y=local/W,l=n/N;
  auto push=[&](uint32_t next,float cost){if(blocked[next]||closed[next])return;float g=a.g+cost;if(g<dist[next]){dist[next]=g;prev[next]=n;q.push({g+1.05f*heuristic(next),g,next});}};
  ...
  if(via[local])for(uint32_t ll=0;ll<8;ll++)if(ll!=l)push(ll*N+local,50);
 }
