...
for block in re.split(r'(?=^\[)',(p/'reports/After_routing_DRC.txt').read_text(),flags=re.M):
 ...
 if width<=.25 or width>1 or name=='PGND':continue
...
k.ZONE_FILLER(b).Fill(b.Zones());k.SaveBoard(str(f),b);(p/'reports/R8_rail_routes.json').write_text(json.dumps({'added':added,'failed':failed,'ampacity':'UNQUALIFIED'},indent=2));print('FINISHED',len(added),len(failed),flush=True)
