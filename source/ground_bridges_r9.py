...
for distance,i,j in pairs:
 ...
 width=4.0 if power(ga) and power(gz) else .6;neck=min(width,.6)
 ...
 for kind,u,v,w in pieces:
  ...
  if any(math.hypot(u[1]-q[1],u[2]-q[2])<.651 for q in newvias):okay=False;break
  newvias.append(u)
...
k.ZONE_FILLER(b).Fill(b.Zones());k.SaveBoard(str(f),b);(p/'reports/Ground_bridges_R9.json').write_text(json.dumps({'added':added,'failed':failed,'groups_remaining':len({root(i) for i in range(len(groups))}),'ampacity':'UNQUALIFIED'},indent=2));print('DONE GROUND',len(added),'groups',len({root(i) for i in range(len(groups))}),flush=True)
