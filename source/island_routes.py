...
for count,block in enumerate(blocks):
 ...
 net=b.FindNet(name).GetNetCode();a=tuple(map(float,ends[0][:2]));z=tuple(map(float,ends[1][:2]));blocked=raster(net,.35);vfree=~raster(net,.65).any(axis=0)
 ...
 b.BuildConnectivity();ia=island(a,net);iz=island(z,net)
...
(p/'reports/Island_routes.json').write_text(json.dumps({'added':added,'failed':fails},indent=2));print('FINISHED',len(added),len(fails),flush=True)
