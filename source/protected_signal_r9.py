...
for block in blocks:
 ...
 for u,v,sh,li in pieces:
  ...
  for t in b.GetTracks():
   ...
   for l in ls:
    ...
    if bb.Intersects(other.BBox()) and k.SHAPE.Collide(sh,other,k.FromMM(.251 if t.GetNetCode() in powercodes else .201)):targets[t.m_Uuid.AsString()]=t;break
 ...
 if len(added)>=14:break
k.ZONE_FILLER(b).Fill(b.Zones());k.SaveBoard(str(f),b);(p/'reports/R8_negotiated_signal.json').write_text(json.dumps({'added':added,'failed':failed,'displaced_items':ripped},indent=2));print('DONE',len(added),'displaced',len(ripped),flush=True)
